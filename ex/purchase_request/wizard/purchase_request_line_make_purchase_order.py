...
class PurchaseRequestLineMakePurchaseOrder(models.TransientModel):
    ...
    def _check_valid_request_line(self, request_line_ids):
        ...
        for line in self.env["purchase.request.line"].browse(request_line_ids):
            if line.request_id.state == "done":
                raise UserError(_("La compra ya ha sido completada."))
            ...
            if not line_picking_type:
                raise UserError(_("Tienes que introducir un Tipo de Picking."))
            if picking_type is not False and line_picking_type != picking_type:
                raise UserError(
                    _("Tiene que seleccionar líneas del mismo Tipo de Picking.")
                )
            else:
    ...
    def check_group(self, request_lines):
        if len(list(set(request_lines.mapped("request_id.group_id")))) > 1:
            raise UserError(
                _(
                    "No puede crear una sola orden de compra desde"
                    "Solicitudes de compra que tienen diferentes grupos de compras."
                )
            )
    ...
    @api.model
    def _prepare_purchase_order(self, picking_type, group_id, company, origin, puchase_request_id):
        ...
        data = {
            ...
            'puchase_request_id': puchase_request_id
        }
        return data
    ...
    def _prepare_purchase_order_line(self, po, item):
        if not item.product_id:
            raise UserError(_("Por favor seleccione un producto para todas las lineas"))
