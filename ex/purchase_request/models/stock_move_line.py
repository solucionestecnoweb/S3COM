...
class StockMoveLine(models.Model):
    ...
    def _purchase_request_confirm_done_message_content(self, message_data):
        ...
        message += _(
            "Los siguientes artículos solicitados de la Solicitud de compra %(request_name)s "
            "ahora se han recibido en %(location_name)s usando Picking %(picking_name)s:"
        ) % {
            "request_name": message_data["request_name"],
    ...
    def _picking_confirm_done_message_content(self, message_data):
        ...
        message += _(
            "Los siguientes artículos solicitados de la Solicitud de compra %(request_name)s "
            "solicitado por %(requestor)s "
            "ahora han sido recibidos en %(location_name)s:"
        ) % {
            "request_name": message_data["request_name"],
