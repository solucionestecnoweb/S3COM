...
class PurchaseOrder(models.Model):
    ...
    puchase_request_id = fields.Many2one(comodel_name='purchase.request', string='requisicion')
    ...
    def _purchase_request_confirm_message_content(self, request, request_dict=None):
        ...
        message += _(
            "Los siguientes artículos solicitados de la Solicitud de compra  %(pr_name)s "
            "ahora han sido confirmados en la orden de comprar %(po_name)s:"
        ) % {
            "po_name": self.name,
            "pr_name": request.name,
        }
    ...
    #             for req in line.purchase_request_lines:
    #                 req.product_qty = line.product_qty
    #                 req.name = line.name
    #                 req.product_id = line.product_id.id
    #             if not line.purchase_request_lines and rec.puchase_request_id:
    #                 self.env['purchase.request.line'].create({
    #                         'product_id': line.product_id.id,
    #                         'name':line.product_id.name,
    #                         "product_uom_id": line.product_uom.id,
    #                         'product_qty':line.product_qty,
    #                         'date_required': rec.date_order,
    #                         'request_id': rec.puchase_request_id.id,
    #                         'purchase_lines': [(6, 0, [line.id])],
