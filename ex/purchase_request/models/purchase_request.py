...
class PurchaseRequest(models.Model):
    ...
    def _compute_is_editable(self):
        for rec in self:
            if rec.state in ("to_approve", "done"):
    ...
    estimated_cost = fields.Monetary(
        ...
        string="Total de Costo Estimado",
        store=True,
    )
    ...
    def unlink(self):
        for request in self:
            if not request._can_be_deleted():
                raise UserError(
                    _("No puede eliminar una requisicion de compra que no sea borrador.")
                )
    ...
    def button_done(self):
        return self.write({"state": "done", "state_so": "closed"}),
    ...
    def to_approve_allowed_check(self):
        for rec in self:
            if not rec.to_approve_allowed:
                raise UserError(
                        _(
                            "No se puede solicitar la aprobación de una solicitud de compra con 0 cantidades. (%s)"
                        )
                        % rec.name
                )
            for line in rec.line_ids:
                if not line.product_id:
                    raise UserError(
                        _("Tiene que seleccionar o crear el producto en las lineas de productos para su aprobacion")
                    )
