    ...
    def get_data(self):
        ...
        for item in self.get_merchandise().sorted(key= lambda x: x.product_id.id):
            ...
            if product != item.product_id.id:
                ...
                seller_n = ''
                ### Vendedores
                for seller in xfind.sorted(key=lambda x: x.order_id.user_id.id):
                    amount = 0
                    if seller_n != seller.order_id.user_id.id:
                        seller_n = seller.order_id.user_id.id
                        ### Cantidad Apartada
                        sfind = self.env['sale.order.line'].sudo().search([
                            ('state', 'in', ('draft', 'sent')),
                            ('product_id', '=', product),
                            ('is_transit_merch', '=', True),
                            ('order_id.user_id', '=', seller_n),
                        ])
                        for line in sfind:
